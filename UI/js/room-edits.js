    ...
    */
    ...
    // Зберігаємо початкові вибрані послуги
    this.initialSelectedServices = new Set();
}
    ...
    this.initialSelectedServices.add(service.service_id);
});
}
}
    ...
    // Перевіряємо чи змінились вибрані послуги
    const currentSelectedServices = new Set(this.selectedServices.keys());
    const servicesChanged = !this.setsEqual(currentSelectedServices, this.initialSelectedServices);

    this.hasChanges = formChanged || servicesChanged;
    this.updateUI();
}
    // Допоміжна функція для порівняння множин
    setsEqual(a, b) {
    ...
    if (saveBtn) {
    // Кнопка завжди активна, крім випадків завантаження
    ...
    // Оновлюємо roomData для розрахунків
    this.roomData.wall_area = wallArea;
    this.roomData.floor_area = floorArea;
    ...
    this.initialSelectedServices.clear();
    ...
    this.initialSelectedServices.add(service.id);
}
});
}
});
    ...
    // Оновлюємо чекбокс та розрахунок в списку послуг
    const serviceElement = document.querySelector(`.service-check[data-service-id="${serviceId}"]`);
    if (serviceElement) {
    serviceElement.checked = false; // Важливо: знім чекбокс
    const serviceItem = serviceElement.closest('.service-item');
    const calculationDetails = serviceItem.querySelector('.calculation-details');
    if (calculationDetails) {
    ...
    // Переконуємося, що total це число
    ...
    serviceElement.innerHTML = `
            ...
            <div class="service-details">
                <span class="service-quantity">${service.quantity.toFixed(2)} м²</span>
                <span class="service-price">${parseFloat(service.price_per_sqm).toFixed(2)} ₴/м²</span>
    ...
    // Оновлюємо початкові вибрані послуги
    this.initialSelectedServices = new Set(this.selectedServices.keys());

    this.hasChanges = false;
    this.updateUI();

    // Перенаправляємо на сторінку кімнат замість показу success модального вікна
    window.location.href = 'http://localhost/BuildMaster/calculator/order-rooms';
